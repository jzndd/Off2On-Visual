...
class OnlineReplayBuffer:
    ...
    def store(self, step, obs, rew, done, act, old_log_prob, state_value):
        ...
        # np.copyto(self.obs[step], obs)
        # np.copyto(self.act[step], act)
        # np.copyto(self.old_log_prob[step], old_log_prob)
        self.obs[step] = obs
        self.act[step] = act
...
class Trainer:
    ...
    def run(self):
        ...
        rb = OnlineReplayBuffer(num_envs, self._cfg.actor_critic.training.batch_size, 
        ...
        # ckpt = torch.load("/data0/jzn/workspace/Off2On-Visual/outputs/2025.05.15/055517_PushCube-v1_ppo_gae2_wholetraj_usebc_use50traj_seed2000_bs12800_minibs256/saved_models/best_100.pth", self._device)
        # self.agent.load_state_dict(ckpt["agent"])
