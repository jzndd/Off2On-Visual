...
class PPOAgent2D(BaseAgent):
    ...
    def predict_act(self, obs: torch.Tensor, eval_mode=False, **kwargss) -> ActorCriticOutput:
        ...
        if self.adv_compute_mode in ['tradition', 'gae2'] and not eval_mode:
    ...
    def update(self, rb: OnlineReplayBuffer, iter=None, max_iter=None):
        ...
        batch = rb.sample_all()
        obss, acts, rews, next_obss, dones, old_log_probs, _, dws = to_torch(batch, self.device)
    ...
    def update_multienv(self, rb: OnlineReplayBuffer, ):
        ...
        with torch.no_grad():
            ...
            for t in reversed(range(num_steps)):
                ...
                delta = rews[t] + 0.9 * real_next_values - old_state_values[t]
                advantages[t] = lastgaelam = delta + 0.9 * self.gae_lambda * next_not_done * lastgaelam # Here actually we should use next_not_terminated, but we don't have lastgamlam if terminated
