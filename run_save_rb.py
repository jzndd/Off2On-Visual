...
def gen_traj(policy, seed, env_name, ep_num, dataset_name, max_path_length=100, 
    ...
    rb = OfflineReplaybuffer(capacity=5000, obs_shape=(3, img_size, img_size))
    # eps_data = []
    eps_imgs = []
    eps_actions = []
    eps_rewards = []
    ...
    while True:
        ...
        if save_whole_traj:
            done = info['success'] or terminated
            end_flag = truncated
        else:
